...
function findMinMax([firstElement, ...restElements]) {
  let minN = [firstElement];
  let maxN = [firstElement];
  for (const i in restElements) {
    if (minN > restElements[i]) {
      minN = restElements[i];
    }

    if (maxN < restElements[i]) {
      maxN = restElements[i];
    }
  }
...
function replaceNumbers(arr) {
  ...
  for (const num in arr) {
    if (num > "10") {
      repl[num] = "Greater than 10";
    } else if (num < "10") {
      repl[num] = "Less than or equal to 10";
    }
  }
